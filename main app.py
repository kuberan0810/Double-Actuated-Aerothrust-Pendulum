...
def BLDC_Ctrl():
    ...
    if data4word[11]=="1":
        ...
        answer = tkMessageBox.askquestion("Turn On Motor", "Do you want to Turn ON motor?", icon='warning')
        if answer == "yes" :
...
def packData():
    ...
    if d4>360 :
        tkMessageBox.showerror('window Title','Enter value between 0 to 360')
    elif d4<0 :
        tkMessageBox.showerror('window Title','Enter value between 0 to 360')
    else :       
        Kp = int(d1*100)
